...
const MeetingsIcon = ({ color }: { color: string }) => (
...
const ActionsIcon = ({ color }: { color: string }) => (
...
const Bar = React.forwardRef<HTMLDivElement, BarProps>((
  ...
  const currentActive: TabVariant = activeTab ?? variant;

  const handleTabPress = (tabId: TabVariant) => {
  ...
  const getIconColor = (tabId: TabVariant): string => {
    if (currentActive === tabId) {
      return activeColor ?? TAB_ACCENT_COLORS[tabId];
    }
    return inactiveColor;
  };

  const renderIcon = (tabId: TabVariant) => {
    const color = getIconColor(tabId);
    switch (tabId) {
      case 'Home':
  ...
  return (
    <MuiBox
      ...
      sx={{
        ...
        height: '74px',
        minHeight: '74px',
        background: 'var(--color-grey-page-light, #091C2A)',
    ...
    >
      ...
      >
        ...
        >
          {/* Decorative accent bg for Home */}
          <MuiBox
            sx={{
              position: 'absolute',
              pointerEvents: 'none',
              background: 'rgba(153,89,255,0.35)',
              opacity: 0.7,
              borderRadius: '12px',
              top: '4px',
              left: '-4px',
          ...
          />
          <MuiBox sx={{ width: '20px', height: '19px', minHeight: '19px', alignItems: 'flex-start', justifyContent: 'flex-start', position: 'relative', zIndex: 1 }}>
            {renderIcon('Home')}
          </MuiBox>
          {showLabels && (
            <MuiTypography
              sx={{
                fontSize: '12px',
                fontWeight: currentActive === 'Home' ? 600 : 400,
                fontFamily: 'Inter',
                lineHeight: 'normal',
                color: currentActive === 'Home'
        ...
        >
          ...
          <MuiBox sx={{ width: '19px', height: '19px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 1 }}>
            {renderIcon('Meetings')}
          </MuiBox>
          {showLabels && (
            <MuiTypography
              sx={{
                fontSize: '12px',
                fontWeight: currentActive === 'Meetings' ? 600 : 400,
                fontFamily: 'Inter',
                lineHeight: 'normal',
                color: currentActive === 'Meetings'
                  ? 'var(--color-white-white, #FFFFFF)'
                  : inactiveColor,
                position: 'relative',
                zIndex: 1,
        ...
        >
          ...
          <MuiBox sx={{ width: '14px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 1 }}>
            {renderIcon('Actions')}
          </MuiBox>
          {showLabels && (
            <MuiTypography
              sx={{
                fontSize: '12px',
                fontWeight: currentActive === 'Actions' ? 600 : 400,
                fontFamily: 'Inter',
                lineHeight: 'normal',
                color: currentActive === 'Actions'
                  ? 'var(--color-white-white, #FFFFFF)'
                  : inactiveColor,
                position: 'relative',
                zIndex: 1,
        ...
        >
          ...
          <MuiBox sx={{ width: '17px', height: '19px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 1 }}>
          ...
          {showLabels && (
            <MuiTypography
              sx={{
                fontSize: '12px',
                fontWeight: currentActive === 'Notifications' ? 600 : 400,
                fontFamily: 'Inter',
                lineHeight: 'normal',
                color: currentActive === 'Notifications'
                  ? 'var(--color-white-white, #FFFFFF)'
                  : inactiveColor,
                position: 'relative',
                zIndex: 1,
